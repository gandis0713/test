...
function CardinalSpline1D(input, spec, output) {
  ...
  this.build = () => {
    ...
    for (let i = 0; i < N; i++) {
      ...
      p4[i] =
        i >= N - 1
          ? input[input.length - 1] + (input[input.length - 1] - input[input.length - 2])
          : input[i + 2];
    }
    ...
    for (let i = 0; i < N; i++) {
      for (let j = 0; j < spec; ++j) {
        ...
        const value =
          this.coeffiA[i] * t3 + this.coeffiB[i] * t2 + this.coeffiC[i] * t1 + this.coeffiD[i];
