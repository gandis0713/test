...
// =====================================================================
// create coordination for panorama by charles
// =====================================================================
vec4 ConvPanoPos2CTPos(vec3 panoTexCoord)
{

// CShaderPanoVolumeDataGetter::SetPanoVolCoordRange charles [ vPanoVolCoordMin ] 
//  [ vec3 ] 0 ,  0.365073 ,  0
// CShaderPanoVolumeDataGetter::SetPanoVolCoordRange charles [ vPanoVolCoordMax ] 
//  [ vec3 ] 1 ,  0.634927 ,  1
// CShaderPanoVolumeDataGetter::SetReorientationTransform charles [ vPanoViewDirRight ] 
//  [ vec3 ] 1 ,  0 ,  0
// CShaderPanoVolumeDataGetter::SetReorientationTransform charles [ vPanoViewDirTargetToEye ] 
//  [ vec3 ] 0 ,  -1 ,  0
// CShaderPanoVolumeDataGetter::SetReorientationTransform charles [ vPanoViewDirUp ] 
//  [ vec3 ] 0 ,  0 ,  -1
// CShaderPanoVolumeDataGetter::SetCurvePlaneNormal charles [ vTempPlaneNormal ] 
//  [ vec3 ] 0 ,  0 ,  1

// CPanoVRMultiDataSep::UpdateCurveInfo charles [ fVolWidth ] 99
// CPanoVRMultiDataSep::UpdateCurveInfo charles [ fVolHeight ] 99
// CPanoVRMultiDataSep::UpdateCurveInfo charles [ fVolDepth ] 79.6

  vec3 vPanoVolCoordMin = vec3(0, 0.365073, 0);
  vec3 vPanoVolCoordMax = vec3(1, 0.634927, 1);

    // vec3 coordPANValid = clamp(panoTexCoord, vPanoVolCoordMin, vPanoVolCoordMax);
    // if(coordPANValid != panoTexCoord)
    //     return 0.;
 
  vec3 vPanoViewDirRight = vec3(1, 0, 0);
  vec3 vPanoViewDirTargetToEye = vec3(0, -1, 0);
  vec3 vPanoViewDirUp = vec3(0, 0, -1);
  vec3 vDirPlaneCurveNormal = vec3(0, 0, 1);

  vec3 vOffsetFromCenter = panoTexCoord - vec3(0.5f, 0.5f, 0.5f);
  float fDisPanoViewRayDir = -1.0f * dot(vPanoViewDirTargetToEye, vOffsetFromCenter);
  float fDisPanoViewUpDir = dot(vPanoViewDirUp, vOffsetFromCenter);
  float fDisPanoViewRightDir = dot(vPanoViewDirRight, panoTexCoord);

  vec4 posCT = texture2D(curveTexture, vec2(fDisPanoViewRightDir, 0.25));
  vec4 vRayDir = texture2D(curveTexture, vec2(fDisPanoViewRightDir, 0.75));
  vRayDir = vRayDir * 2.0f - vec4(1, 1, 1, 0);
  posCT = posCT + vRayDir * fDisPanoViewRayDir;
  posCT = posCT - vec4(vDirPlaneCurveNormal, 0) * fDisPanoViewUpDir;
...
{
  // opacity input range = 0.0 ~ 1.0
  // contrast input range = -1.0 ~ 1.0
  // brightnes input range = -1.0 ~ 1.0

  // change range
  float newContrast = ( 1.0 - ( contrast / 2.0 )); // range 0.5 ~ 1.5
  float newBrightnes = (-brightness + 1.0) / 2.0; // range 0 ~ 1

  float minValue = newBrightnes - ( newContrast * 0.5 );
  float a = 1.0 / newContrast;
  float b = 0.0 - (a * minValue);
  
  color.r = clamp(a * color.r + b, 0., 1.);
  color.g = clamp(a * color.g + b, 0., 1.);
  color.b = clamp(a * color.b + b, 0., 1.);

  color.a = clamp(opacity * color.a, 0., 1.);
...
void main()
{
  ...
  // apply opacity, brightness and contrast by input value;
