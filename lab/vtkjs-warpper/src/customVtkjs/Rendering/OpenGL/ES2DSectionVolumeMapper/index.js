...
import vtkVolumeFS from '../glsl/vtkVolumeFS2DSection.glsl';

const { vtkWarningMacro, vtkErrorMacro } = macro;

// ----------------------------------------------------------------------------
// vtkES2DSectionOpenGLVolumeMapper methods
// ----------------------------------------------------------------------------

function vtkES2DSectionOpenGLVolumeMapper(publicAPI, model) {
  // Set our className
  model.classHierarchy.push('vtkES2DSectionOpenGLVolumeMapper');
  ...
  publicAPI.updateShaders = (cellBO, ren, actor) => {
    ...
    if (publicAPI.getNeedToRebuildShaders(cellBO, ren, actor)) {
      ...
      const newShader = model.openGLRenderWindow
        .getShaderCache()
        .readyShaderProgramArray(shaders.Vertex, shaders.Fragment, shaders.Geometry);
  ...
  publicAPI.setCameraShaderParameters = (cellBO, ren, actor) => {
    ...
    program.setUniformf('clipWidth', model.renderable.getClipWidth());
    program.setUniformf('clipHeight', model.renderable.getClipHeight());
    ...
    for (let i = 0; i < 6; ++i) {
      ...
      // if (actor.getProperty().getUseLabelOutline())
      {
        ...
        program.setUniformf('vpWidth', size[0]);
        program.setUniformf('vpHeight', size[1]);
        program.setUniformi('filteringMode', model.renderable.getFilteringMode());
  ...
  publicAPI.setPropertyShaderParameters = (cellBO, ren, actor) => {
    ...
    if (iComps && numComp >= 2) {
      ...
      for (let i = 0; i < numComp; ++i) {
        program.setUniformf(`mix${i}`, actor.getProperty().getComponentWeight(i) / totalComp);
      }
    }
...
export function extend(publicAPI, model, initialValues = {}) {
  ...
  // Object methods
  vtkES2DSectionOpenGLVolumeMapper(publicAPI, model);
}

// ----------------------------------------------------------------------------

export const newInstance = macro.newInstance(extend, 'vtkES2DSectionOpenGLVolumeMapper');
