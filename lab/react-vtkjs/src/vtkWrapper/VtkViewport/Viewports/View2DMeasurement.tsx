...
export interface View2DMeasurementApis {
  uid: string;
...
interface View2DMeasurementProps {
  volumeData: vtkImageData;
  sliceRange: number[];
  currentSlice: number;
  orientation?: SliceOrientation;
  onDestroyed?: () => void;
  onCreated?: (api2D: View2DMeasurementApis) => void;
...
export default class View2DMeasurement extends Component<View2DMeasurementProps, ViewState> {
  ...
  constructor(props) {
    ...
    this.onCurveCreated = this.onCurveCreated.bind(this);
  ...
  componentDidMount(): void {
    ...
    if (this.props.onCreated) {
       ...
       */
      const api: View2DMeasurementApis = {
  ...
  componentDidUpdate(prevProps: View2DMeasurementProps): void {
    ...
    if (!this.glWindow.getContainer()) {
      ...
      this.splineHandle.setCurveCreatedCallback(this.onCurveCreated);
      console.log('contSize.width : ', contSize.width);
      console.log('contSize.height : ', contSize.height);
  ...
  onCurveCreated(curve: PanoCurve): void {
    panoCurveState.data = curve.data;
    panoCurveState.normal = curve.normal;
    panoCurveState.length = curve.length;
    panoCurveState.sectionCenter = Math.floor(panoCurveState.data.length / 2.0);

    const boundZ: number =
      this.props.volumeData.getBounds()[5] - this.props.volumeData.getBounds()[4];
    panoCurveState.height = boundZ;

    console.log('panoCurveState.height : ', panoCurveState.height);

    if (this.props.onCreatePanorama) {
      this.props.onCreatePanorama();
    }
  }
  getOrientation(): SliceOrientation | undefined {
  ...
  setupTopRulerState(): void {
    ...
    this.rulerStateTop.positionMargin = 20;
  }
  ...
  updateImage(): void {
    console.log(`updateImage@View2DMeasurement`);
    this.renderWindow.render();
  }
  ...
  render(): React.ReactElement {
    ...
    return (
      <div style={style}>
        ...
        <ViewportOverlay
          botRight={GetSliceInfoString(this.props.sliceRange[1], this.props.currentSlice)}
        ...
        {this.container.current ? (
          <div>
            <VTRuler name={'VTTopRuler'} rulerState={this.rulerStateTop} ref={this.refRulerTop} />
            <VTRuler
              name={'VTRightRuler'}
