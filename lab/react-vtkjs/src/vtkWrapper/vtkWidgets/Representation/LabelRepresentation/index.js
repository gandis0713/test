...
function vtkLabelRepresentation(publicAPI, model) {
  ...
  publicAPI.computeInteractionState = pos => {
    if (model.canvas) {
      const { height } = model.canvas;
      const { width } = model.canvas;
  ...
  publicAPI.computeTextDimensions = text => {
    const currentLabelStyle = model.highlight ? model.selectLabelStyle : model.labelStyle;
    ...
    const lineSpace = currentLabelStyle.fontSize * (1 + currentLabelStyle.lineSpace);

    const padding = currentLabelStyle.fontSize / 4;

    const height = 2 * padding + currentLabelStyle.fontSize + (lines.length - 1) * lineSpace;

    const width = lines.reduce(
      (maxWidth, line) => Math.max(maxWidth, Math.round(model.context.measureText(line).width)),
      0
    );
  ...
  publicAPI.updateLabel = () => {
    if (model.context && model.canvas) {
      ...
      if (model.actor.getVisibility()) {
        ...
        const { width, height, lineSpace, padding, lines } = publicAPI.computeTextDimensions(
          model.labelText
        );
...
export function extend(publicAPI, model, initialValues = {}) {
  ...
  model.mapper.setCallback(coordList => {
    if (model.canvas) {
      let xOffset = 0;
      if (model.textAlign === TextAlign.RIGHT) {
        xOffset = -model.canvas.width;
      } else if (model.textAlign === TextAlign.CENTER) {
        xOffset = -0.5 * model.canvas.width;
      }
