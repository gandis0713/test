...
function vtkInteractorStyleMPRSlice(publicAPI, model) {
  ...
  function isCameraViewInitialized(camera) {
    ...
    const bInited = typeof dist === 'number' && dist === Number(dist) && Number.isFinite(dist);
    console.log(`bInited${bInited}`);
  ...
  function setSliceNormalInternal(normal) {
    ...
    if (model.volumeActor) {
      ...
      if (isCameraViewInitialized(camera) === false) {
        console.log('init camera');
        const bounds = model.volumeActor.getBounds();
  ...
  publicAPI.setViewport = viewportData => {
    ...
    if (viewportData) {
      ...
      viewportData.getEventWindow().addEventListener(EVENTS.VIEWPORT_ROTATED, onRotateChanged);
    }
  };
  ...
  publicAPI.superHandleMiddleButtonRelease = publicAPI.handleMiddleButtonRelease;
  ...
  publicAPI.getTotalSliceCount = () => {
    publicAPI.getSliceRange();
    return cache.sliceRange[1] - cache.sliceRange[0] + 1;
  };

  publicAPI.getCurrentSliceIndex = () => {
    return publicAPI.getSlice() - cache.sliceRange[0];
  };

  publicAPI.setCurrentSliceIndex = index => {
    const range = publicAPI.getSliceRange();
    const midPoint = (range[1] - range[0]) / 2.0;
    publicAPI.setSlice(index - midPoint);
  };
  ...
  publicAPI.setSliceOrientation = (normal, viewUp) => {
    console.log('setrSliceOrientation');
    model.initialized = false;
    const viewportData = publicAPI.getViewport();
...
export function extend(publicAPI, model, initialValues = {}) {
  ...
  macro.get(publicAPI, model, ['slabThickness', 'volumeActor', 'apis', 'apiIndex']);
