...
vec4 get2DPanCoord(float ratio, float dir)
{
  vec4 posCT = texture2D(curveTexture, vec2(ratio, 0.25f));
  vec4 vRayDir = texture2D(curveTexture, vec2(ratio, 0.75f));
  vRayDir.x = vRayDir.x * dir;
  vRayDir.y = vRayDir.y * dir;
  posCT = posCT + vRayDir * 0.3;
...
// position
//
void compute2DpanIndexSpaceValues(out vec3 pos, out vec3 endPos, out float sampleDistanceIS)
{
  pos = get2DPanCoord((vertexDCOutput.x + 1.0) / 2.0, 1.0).xyz + vec3(0, 0, 1.0 - (vertexDCOutput.y + 1.0) / 2.0);
  endPos = get2DPanCoord((vertexDCOutput.x + 1.0) / 2.0, -1.0).xyz + vec3(0, 0, 1.0 - (vertexDCOutput.y + 1.0) / 2.0);
  
  sampleDistanceIS = 0.008;
}
...
void main()
{
  ...
  compute2DpanIndexSpaceValues(posIS, endIS, sampleDistanceIS);
