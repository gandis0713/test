...
const { vtkWarningMacro, vtkErrorMacro, normalizeWheel, vtkOnceErrorMacro } = macro;
...
function vtkRenderWindowInteractor(publicAPI, model) {
  ...
  publicAPI.cancelAnimation = (requestor, skipWarning = false) => {
    if (!animationRequesters.has(requestor)) {
      if (!skipWarning) {
        const requestStr =
          requestor && requestor.getClassName ? requestor.getClassName() : requestor;
  ...
  publicAPI.recognizeGesture = (event, positions) => {
    ...
    let originalAngle = vtkMath.degreesFromRadians(
      Math.atan2(startVals[1].y - startVals[0].y, startVals[1].x - startVals[0].x)
    );
