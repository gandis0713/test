...
import vtkResliceCursorContextRepresentation from '../../Representations/ResliceCursorContextRepresentation';
...
import { boundPlane, updateState, getViewPlaneNameFromViewType } from './helpers';
...
function vtkResliceCursorWidget(publicAPI, model) {
  ...
  function updateCamera(renderer, normal) {
    ...
    const estimatedCameraPosition = vtkMath.multiplyAccumulate(focalPoint, normal, distance, [
      0,
      0,
      0
    ]);
    ...
    const newCameraPosition = vtkMath.multiplyAccumulate(newFocalPoint, normal, distance, [
      0,
      0,
      0
    ]);
    ...
    renderer
      .getActiveCamera()
      .setPosition(newCameraPosition[0], newCameraPosition[1], newCameraPosition[2]);
  ...
  publicAPI.updateReslicePlane = (imageReslice, viewType) => {
    ...
    modified =
      imageReslice.setOutputOrigin([0.5 * outputSpacingX, 0.5 * outputSpacingY, 0]) || modified;
