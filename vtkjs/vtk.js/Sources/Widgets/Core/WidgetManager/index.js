...
function vtkWidgetManager(publicAPI, model) {
  ...
  function getViewWidget(widget) {
    return (
      widget &&
      (widget.isA('vtkAbstractWidget') ? widget : widget.getWidgetForView({ viewId: model.viewId }))
    );
  }
  ...
  function updateSvg() {
    if (model.useSvgLayer) {
      for (let i = 0; i < model.widgets.length; i++) {
        ...
        const svgReps = widget.getRepresentations().filter(r => r.isA('vtkSVGRepresentation'));

        let pendingContent = [];
        if (widget.getVisibility()) {
          pendingContent = svgReps.filter(r => r.getVisibility()).map(r => r.render());
        }
  ...
  publicAPI.setRenderer = renderer => {
    ...
    subscriptions.push(model.openGLRenderWindow.onModified(updateDisplayScaleParams));
  ...
  publicAPI.updateSelectionFromMouseEvent = event => {
    ...
    const { top, left, height } = model.openGLRenderWindow.getCanvas().getBoundingClientRect();
    const x = pageX - left;
