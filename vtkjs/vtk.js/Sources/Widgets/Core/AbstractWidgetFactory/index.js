...
import { extractRenderingComponents } from '../WidgetManager';
...
function vtkAbstractWidgetFactory(publicAPI, model) {
  ...
  publicAPI.getWidgetForView = ({ viewId, renderer, viewType, initialValues }) => {
    if (!viewToWidget[viewId]) {
      ...
      const { interactor, openGLRenderWindow, camera } = extractRenderingComponents(renderer);
      const widgetModel = {};
